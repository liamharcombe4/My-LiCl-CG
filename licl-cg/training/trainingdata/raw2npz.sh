...
python3 -c '\
import numpy as np; cell = np.load("cell.npy"); z = np.load("z.npy"); R = np.load("R.npy"); F = np.load("F.npy"); pbc = np.load("pbc.npy");
np.savez("dataLiCl.npz", cell=cell, z=z, R=R, F=F, pbc=pbc)'
